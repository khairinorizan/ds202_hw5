...
top5Category <- categoryBreakdown %>% top_n(5)

top5MonthlyBreakdown <- categoryMonthBreakdown %>% filter(`Category Name` %in% top5Category$`Category Name`) %>% arrange(desc(`Category Name`))

ggplot(top5MonthlyBreakdown, aes(x=Month, y=n)) +
  geom_line() + facet_wrap(~`Category Name`) + labs(y = "Count")
```
